...
export default class ProfileStore {
  ...
  @action loadUserActivities = async (username: string, predicate?: string) => {
    ...
    try {
      const activities = await Service.Profiles.listActivities(
        username,
        predicate!
      )
      runInAction(() => {
  ...
  @action follow = async (username: string) => {
    this.loading = true
    try {
      await Service.Profiles.follow(username)
      runInAction(() => {
  ...
  @action unfollow = async (username: string) => {
    this.loading = true
    try {
      await Service.Profiles.unfollow(username)
      runInAction(() => {
  ...
  @action loadFollowings = async (predicate: string) => {
    this.loading = true
    try {
      const profiles = await Service.Profiles.listFollowings(
